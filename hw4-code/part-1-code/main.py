...
def create_augmented_dataloader(args, dataset):
    ...
    #1. Select 5000 random examples from the original training set
    original_train_set = dataset["train"]
    # Shuffle and select 5000 examples
    transformed_subset = original_train_set.shuffle(seed=42).select(range(5000))

    # 2. Apply the transformation to the selected subset
    # 'custom_transform' is assumed to be implemented in utils.py
    transformed_dataset = transformed_subset.map(custom_transform, load_from_cache_file=False)

    # 3. Combine the original training set and the transformed dataset
    # The original training set is dataset["train"]
    augmented_dataset = datasets.concatenate_datasets([original_train_set, transformed_dataset])

    # 4. Tokenize the augmented dataset
    augmented_tokenized_dataset = augmented_dataset.map(tokenize_function, batched=True, load_from_cache_file=False)

    # 5. Prepare the dataset format (remove text column, rename label, set format)
